...
class DayView extends LinearLayout {
    ...
    public DayView(Context context, CalendarDay day) {
        ...
        tv_day = new CheckedTextView(getContext());
        LayoutParams params = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
        setOrientation(LinearLayout.VERTICAL);
        tv_day.setLayoutParams(params);
        tv_day.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16);
        setDay(day);

        tv_subject = new CheckedTextView(getContext());
        LayoutParams params1 = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
        params1.topMargin = -dip2px(getContext(), 6);
        tv_subject.setLayoutParams(params1);
        tv_subject.setText("");
        tv_subject.setTextSize(TypedValue.COMPLEX_UNIT_SP, 10);


        iv_done= new ImageView(getContext());
        iv_done.setLayoutParams(params);
        iv_done.setVisibility(View.INVISIBLE);
        iv_done.setImageResource(R.drawable.ic_caldroid_today_right);

        ColorStateList colorStateList = getContext().getResources().getColorStateList(R.color.mcv_text_date_light);
        tv_day.setTextColor(colorStateList);
        tv_subject.setTextColor(colorStateList);
    ...
    public void setDayFormatter(DayFormatter formatter) {
        ...
        CharSequence currentLabel = tv_day.getText();
        Object[] spans = null;
    ...
    private void setEnabled() {
        ...
        if (!isInMonth && shouldBeVisible) {
            tv_day.setTextColor(tv_day.getTextColors().getColorForState(
...
//        tv_day.setTextAppearance(context, taId);
//        tv_subject.setTextAppearance(context, taId);
    }

    @Override
    public void setSelected(boolean selected) {
        super.setSelected(selected);
        tv_day.setChecked(selected);
        tv_subject.setChecked(selected);
    }

    //设置是否显示学科
    public void showSubjectText(String text) {
    ...
    //设置是否显示已做的图片
    public void showDoImg(boolean show, boolean right) {
        iv_done.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
        if(right) {
            if(isSelected()) {
                iv_done.setImageResource(R.drawable.ic_caldroid_today_right);
            } else {
                iv_done.setImageResource(R.drawable.ic_caldroid_day_right);
            }
        } else {
            iv_done.setImageResource(R.drawable.ic_caldroid_day_wrong);
        }
    }
}
