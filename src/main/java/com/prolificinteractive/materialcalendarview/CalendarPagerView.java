...
abstract class CalendarPagerView extends ViewGroup implements View.OnClickListener {
    ...
    public void showSubject(List<SubjectCalendar> subjectCalendars) {
        if(subjectCalendars == null) return;
        for (DayView dayView : dayViews) {
//            dayView.showSubjectText(true);
            CalendarDay dayCalendar = dayView.getDate();
            for (SubjectCalendar subjectCalendar : subjectCalendars) {
                if(dayCalendar.equals(subjectCalendar.calendarDay)) {
                    dayView.showSubjectText(subjectCalendar.subjectName);
                    dayView.showDoImg(subjectCalendar.done, subjectCalendar.right);
                }
            }
        }
    }
